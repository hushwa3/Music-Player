...
export class StreamingService {
  ...
  private async loadToken() {
    this.authToken = await this.dataService.get('spotify_token');
    this.tokenExpiration = await this.dataService.get('spotify_token_expiration') || 0;
  }
  ...
  private saveToken(token: string, expiresIn: number) {
    ...
    this.dataService.set('spotify_token', token);
    this.dataService.set('spotify_token_expiration', expiration);
  }
